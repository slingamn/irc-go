...
var (
	...
	// ErrorTagsContainBadChar indicates that the tags contained invalid characters
	ErrorTagsContainBadChar = errors.New("Tags contain invalid characters")
...
func (ircmsg *IrcMessage) parseTags(tags string) (err error) {
	for 0 < len(tags) {
		...
		unescapedValue, err := UnescapeTagValue(tagValue)
		if err != nil {
			return ErrorTagsContainBadChar
		}
		ircmsg.SetTag(tagName, unescapedValue)
...
func (ircmsg *IrcMessage) line(fromClient bool, truncateLen int) ([]byte, error) {
	...
	// TODO evaluate buf.Grow(512) here
